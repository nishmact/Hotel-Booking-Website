...
const Notifications = () => {
  ...
  const fetchNotification = async (page: number) => {
    axiosInstanceAdmin
      .get(`${BASE_URL}/api/admin/admin-notifications?recipient=${admin?._id}&page=${page}`, {
  ...
  const handleRead = async (
    ...
    axiosInstanceAdmin
      .patch(
        `${BASE_URL}/api/admin/toggle-read`,
  ...
  const handleDelete = async (
    ...
    e.preventDefault();
    axiosInstanceAdmin
      .delete(`${BASE_URL}/api/admin/notification?id=${id}&recipient=${admin?._id}`, {
        withCredentials: true,
      })
      .then((response) => {
        fetchNotification(currentPage)
      ...
      {notifications?.length > 0 ? (
        <div className="col-span-6 xl:col-span-4 mx-10 lg:mx-20">
          ...
          {notifications?.map((data, key) => (
            <div
              className="block rounded-sm border border-warning bg-white mb-4 shadow-md hover:shadow-lg dark:border-strokedark dark:bg-boxdark"
              key={key}
            >
              <div
                className={`${
                  !data.read
                    ? "bg-gray-400 p-4 bg-opacity-30"
                    : "bg-gray-100 p-4 bg-opacity-30"
                }`}
              >
